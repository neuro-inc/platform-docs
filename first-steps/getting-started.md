...
Для того, чтобы Jupyter Notebooks корректно работали, скрипт `train.py` и сами блокноты должны быть доступны на дисковом пространстве платформы. Загрузите папку `code`, содержащую этот файл, на дисковое пространство платформы с помощью такой команды:

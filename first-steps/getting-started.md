...
[Веб-терминал](https://apps.neu.ro/shell?cluster\_name=neuro-compute) не требует установки и может быстро познакомить вас с платформой, позволяя работать в браузере.
...
Создадим каталог `demo` в корне доступного вам сетевого хранилища:

```
$ neuro mkdir -p storage:demo
```

Запустим команду, которая монтирует каталог `demo` на сетевом хранилище к папке `/demo` в контейнере задания и создает в нём файл:

```
$ neuro run --volume storage:demo:/demo:rw ubuntu -- bash -c "echo Hello >> /demo/hello.txt"
```

Убедитесь, что файл, который вы только что создали, действительно находится на сетевом хранилище:
...
Разработка с помощью Jupyter Notebooks является хорошим примером использования платформы. Хотя сеанс Jupyter Notebooks можно запустить с помощью одной команды в командной строке или одним щелчком мыши в веб-интерфейсе, мы рекомендуем вести разработку на основе проектов. Чтобы упростить процесс, мы предоставляем шаблон проекта, основанный на [пакете cookiecutter](https://github.com/cookiecutter/cookiecutter) и являющийся частью **Neu.ro Toolbox.** Этот шаблон предоставляет базовую структуру папок и интеграцию с несколькими рекомендуемыми инструментами.

### Создание нового проекта cookiecutter

Для начала, вам потребуется установить пакет **cookiecutter** через **pip** или **pipx**:
...
├── .neuro.toml         <- автоматически сгенерированный конфигурационный файл для Neuro CLI
├── .neuroignore        <- файл, указывающий Neuro CLI, какие файлы игнорировать при загрузке на хранилище платформы
├── HELP.md             <- автоматически сгенерированная справка шаблона
├── README.md           <- автоматически сгенерированный информационный файл
├── Dockerfile          <- описание Docker-образа, использующегося для обучения в вашем проекте
...
└── update_actions.py   <- скрипт для обновления neuro-flow actions в одном из рабочих процессов GitHub
```

Шаблон содержит файл конфигурации `.neuro/live.yaml` для `neuro-flow`. Этот файл обеспечивает связь между указанной выше структурой файлов и каталогов, базовым рабочим окружением, которое мы предоставляем и манипуляциями с дисковым пространством и заданиями. Например, с помощью команды `upload` подпапки на Вашем локальном компьютере синхронизируются с подпапками на сетевом хранилище, а подпапки на сетевом хранилище синхронизируются с соответствующими подпапками в контейнерах заданий.
...
Для того, чтобы Jupyter Notebooks корректно работали, скрипт `train.py` и сами блокноты должны быть доступны на сетевом хранилище платформы. Загрузите папку `code`, содержащую этот файл, на сетевое хранилище платформы с помощью такой команды:
...
Теперь, когда Вы пишете программу в Jupyter Notebooks, она сохраняется на сетевом хранилище. Чтобы сохранить ваши блокноты Jupyter локально (например, для связи с системой контроля версий), введите команду:

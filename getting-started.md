...
When you have it up and running, you can run the following commands in Conda Prompt:  
...
To make sure that all the commands you can find in our documentation work properly, don't forget running _**bash**_ every time you start the Conda Prompt.
...
Run a job in Neu.ro default environment \(`neuromation/base`\) on GPU which checks if CUDA is available in this environment:

```text
neuro run --preset gpu-k80-small --name test neuromation/base python -c "import torch; print(torch.cuda.is_available());"
```

To check all available CPU and GPU presets, run:
...
The template contains `neuro/live.yaml` configuration file for `neuro-flow`, which guarantees the contract between the above-shown structure, the base environment which we provide, and manipulations with storage and jobs. For example, through the `upload` command, sub-folders on your local machine are synchronized with sub-folders on persistent platform storage, and those sub-folders are synced with the corresponding sub-folders in job containers.

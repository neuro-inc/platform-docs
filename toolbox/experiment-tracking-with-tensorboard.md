...
* Классификатор и матрица неточностей созданы, и теперь нам нужно логировать базовые показатели и матрицу неточностей в конце каждого цикла. Заметьте, что мы выбрали папку `results` для логирования результатов. Вы можете выбрать другую папку для этих целей, если потребуется.
...
Вкладка **Scalars** показывает, как точность и потери изменяются с каждой эпохой. Здесь можно следить за скоростью обучения, темпом обучаемости и другими показателями. Вы можете увидеть больше информации, наводя курсор на график.

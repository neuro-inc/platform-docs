# Hyperparameter Tuning with Weights & Biases — NEW
...
Now you can run `develop`, `train`, `jupyter` and other jobs and pass them the following parameter to use W&B:

```text
make develop SECRETS="wandb=wandb-key"
make train SECRETS="wandb=wandb-key"
make jupyter SECRETS="wandb=wandb-key"
```

Neu.ro authenticates W&B via your API key by running `wandb login` at job’s startup.
...
```text
make hypertrainSECRETS="wandb=wandb-key"
```
...
```text
make hypertrain N_JOBS=10 SECRETS="wandb=wandb-key"
```

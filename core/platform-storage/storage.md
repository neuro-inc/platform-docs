...
Платформа предоставляет сетевое хранилище для размещения файлов, необходимых для выполнения Ваших задач. Все пользователи имеют доступ к сетевому хранилищу для размещения данных, программного кода и артефактов. Каждый объем сетевого хранилища относится к своему кластеру и Вы не можете использовать файлы между кластерами. Управлять сетевым хранилищем можно из веб-интерфейса или из командной строки CLI.

На нижеприведенном рисунке показан поток данных между Вашим локальным компьютером, сетевым хранилищем платформы и файловой системой заданий:

![](../../.gitbook/assets/storage-2.png)

## Структура сетевого хранилища платформы

Сетевое хранилище платформы представляет собой древоподобную файловую систему, и у каждого кластера есть своё сетевое хранилище.

Корневые папки всех пользователей кластера находятся в корне сетевого хранилища этого кластера. Таким образом, если на кластере есть три пользователя `usera`, `userb` и `userc`, в корневом каталоге будут находиться три папки `/usera`, `/userb` и `/userc` :
...
Общий синтаксис доступа к файлам и папкам на сетевом хранилище через CLI таков:
...
## Проверка использования сетевого хранилища

Вы можете получить информацию об использовании сетевого хранилища на текущем кластере, запуская команду `neuro storage df`:
...
* Нажмите **ЗАПУСТИТЬ** в виджете **Браузер файловой системы**.

![](<../../.gitbook/assets/зображення (16) (1).png>)

* Нажмите **ЗАПУСТИТЬ** в новом диалоговом окне**.**

![](<../../.gitbook/assets/зображення (11).png>)

Откроется менеджер файлов для кластера, посредством которого Вы можете управлять файлами.

![Интерфейс FileBrowser](<../../.gitbook/assets/image (18).png>)
...
* Опции: В этой области можно создать новую папку или файл и управлять настройками сетевого хранилища.&#x20;
...
### Копирование файлов между сетевым хранилищем платформы и Вашей локальной файловой системой
...
* Для копирования файлов с локального компьютера на сетевое хранилище платформы используйте следующую команду: `neuro cp parameters.txt storage:nero-assistant`. Эта команда копирует файл `parameters.txt` с локального хоста в папку `nero-assistant` на диске платформы.
...
* Чтобы скопировать файлы с платформы на локальный компьютер, используйте команду: `neuro cp storage:nero-assistant/TestResults.txt file:///Projects` Данная команда копирует файл `TestResults.txt` в папке `nero-assistant`, находящейся на хранилище платформы, на локальный компьютер.
...
* Чтобы скопировать папку из локальной хост-системы на сетевое хранилище платформы, используйте команду: `neuro cp -r samplecode storage:nero-assistant`. Данная команда копирует папку `samplecode` и все ее содержимое в папку `nero-assistant` на сетевом хранилище платформы.
...
* Чтобы скопировать папку с платформы на локальный хост используйте команду: `neuro cp -r storage:nero-assistant/testlogs file:///Projects`. Данная команда копирует папку `testlogs` на сетевом хранилище платформы, со всем её содержимым, на локальный компьютер.
...
Для удаления файлов с сетевого хранилища платформы используйте команду `neuro rm [Files/Folder]`
...
* Для удаления файла с сетевого хранилища платформы используйте команду `neuro rm storage:nero-assistant/parameters.txt`
* Для удаления папки с сетевого хранилища платформы используйте команду `neuro rm -r storage:nero-assistant/samplecode`
...
### Перемещение файлов и папок внутри сетевого хранилища платформы
...
* Чтобы переместить файл на сетевом хранилище в другую папку и переименовать его, используйте команду `neuro mv storage:nero-assistant\code.txt storage:neuro-tutorial\samplecode.txt`. Эта команда перемещает файл `code.txt` из папки `nero-assistant` в папку `neuro-tutorial` и переименовывает файл в `samplecode.txt`.
* Чтобы переместить папку и ее содержимое на сетевом хранилище на новое местоположение, используйте команду: `neuro mv -T storage:nero-assistant/ModelCode storage:neuro-tutorial/SampleCode`. Данная команда копирует папку `ModelCode` и все ее содержимое из папки `nero-assistant` в папку `neuro-tutorial` и переименовывает её в `SampleCode`.

## Как я могу использовать папки на сетевом хранилище в заданиях?

Папки на сетевом хранилище можно использовать в заданиях. Для этого надо подключить каталог на диске к контейнеру, на котором выполняется задание.

### **Замечания**:

* Задания могут выполняться медленнее при использовании папок на сетевом хранилище по сравнению с использованием файловой системы контейнера. Наши эксперименты показывают, что обучение моделей CV занимает на 10-20% больше времени, когда набор данных считывается с примонтированного каталога, расположенного на сетевом хранилище платформы. Если такая разница для Вас критична, мы рекомендуем перед выполнением задания загрузить наборы данных в не примонтированный каталог.
* Настоятельно не рекомендуется распаковывать наборы данных (ZIP, TAR и т.д.) в каталог на сетевом хранилище платформы, поскольку это занимает много времени.

&#x20;При работе с сетевым хранилищем есть несколько ключевых моментов:
...
Первая часть - `storage:/путь/на/платформе` - определяет путь по отношению к корневой папке сетевого хранилища платформы. Вы также можете указать полный URI, включающий в себя название кластера: `storage://кластер/путь/на/хранилище/платформы`.&#x20;
...
## Операции с сетевым хранилищем на других кластерах

Все операции, описанные выше, можно выполнять с сетевым хранилищем на других кластерах.&#x20;

Например, чтобы скопировать локальный файл `parameters.txt` на сетевое хранилище кластера `<имя-кластера>`, запустите:
